...
class ScratchpadBank(n: Int, w: Int, aligned_to: Int, single_ported: Boolean, use_shared_ext_mem: Boolean, is_dummy: Boolean) extends Module {
  ...
  val fromRouter = io.read.req.bits.fromRouter
  ...
  q.io.enq.bits.fromRouter := RegNext(fromRouter)
  ...
  class Impl extends LazyModuleImp(this) with HasCoreParameters {
    ...
    val spad_mems = {
      ...
      bank_ios.zipWithIndex.foreach { case (bio, i) =>
        ...
        val Rourter_read_req = io.Router_srams.read(i).req
        val exread = ex_read_req.valid
        val Routerread = Rourter_read_req.valid
        ...
        bio.read.req.valid := exread || dmawrite || Routerread
        ex_read_req.ready := bio.read.req.ready
        Rourter_read_req.ready := bio.read.req.ready && !exread && !dmawrite
        ...
        when (exread) {
          ...
          bio.read.req.bits.fromRouter := false.B
        }.elsewhen (dmawrite) {
          ...
          bio.read.req.bits.addr := Rourter_read_req.bits.addr
          bio.read.req.bits.fromDMA := false.B
          bio.read.req.bits.fromRouter := true.B
        }.otherwise {
        ...
        val Router_read_resp = Wire(Decoupled(new ScratchpadReadResp(spad_w)))
        Router_read_resp.valid := bio.read.resp.valid && bio.read.resp.bits.fromRouter
        Router_read_resp.bits := bio.read.resp.bits

        val dma_read_pipe = Pipeline(dma_read_resp, spad_read_delay)
        val ex_read_pipe = Pipeline(ex_read_resp, spad_read_delay)
        val Router_read_pipe = Pipeline(Router_read_resp, spad_read_delay)

        bio.read.resp.ready := Mux(bio.read.resp.bits.fromRouter, Router_read_resp.ready, Mux(bio.read.resp.bits.fromDMA, dma_read_resp.ready, ex_read_resp.ready))
        ...
        io.Router_srams.read(i).resp <> Router_read_pipe
      }

      // Writing to the SRAM banks
      bank_ios.zipWithIndex.foreach { case (bio, i) =>
        val exwrite = io.srams.write(i).en
        val Routerwrite = io.Router_srams.write(i).en
        ...
        bio.write.en := exwrite || dmaread || zerowrite || Routerwrite
        ...
        }.elsewhen (Routerwrite){
          bio.write.addr := io.Router_srams.write(i).addr
          bio.write.data := io.Router_srams.write(i).data
          bio.write.mask := io.Router_srams.write(i).mask
